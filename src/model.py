...
class CASunGroup:
    ...
    def _update_sun(self, step: bool = True):
        if step:
            self.sun_optim.step()
        # Gradients are always cleared; when step=False they are intentionally discarded.
    ...
    def _run_competition_parallel(
        ...
        strengths = strengths.masked_fill(~alive_mask, _NEG_INF)
        ...
        x_new[:, :, self.ali_idxs] = x_new[:, :, self.ali_idxs] / (
            reduce(x_new[:, :, self.ali_idxs], "n b c h w -> n b 1 h w", "sum").clamp_min(1e-8)
        ).to(x_new.dtype)
        return x_new
    ...
    def load(self, loc: str) -> bool:
        ...
        before_state = {k: v.clone() for k, v in self.models.model.state_dict().items()}
        ...
        after_state = self.models.model.state_dict()
        any_changed = any(
            not torch.equal(before_state[k], after_state[k]) for k in before_state
        )
        if not any_changed:
