...
class World:
    ...
    def save(self, config: Config, run_name: str) -> None:
        if hasattr(self, "seed_update"):
            seed = self.seed_update.float().detach().cpu().numpy()
        else:
            seed = np.zeros((config.n_ncas, self.seed_dim), dtype=np.float32)
        np.save(f"{run_name}/seed.npy", seed)
